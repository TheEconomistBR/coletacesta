...
# Função para conectar ao banco de dados de login
...
# Função para inicializar o banco de dados de login
def init_login_db():
    if not os.path.exists('banco_de_dados/login.db'):
...
@app.route('/delete_price', methods=['POST'])
def delete_price():
    if 'user_id' not in session:
        return redirect(url_for('login'))
    ...
    conn = get_db_connection()
    conn.execute('DELETE FROM preco_produtos WHERE id = ?', (price_id,))
    conn.commit()
    conn.close()

    flash('Registro de preço excluído com sucesso!', 'success')
    return redirect(url_for('index'))
...
# Certifique-se de chamar init_login_db() e init_db() quando iniciar o aplicativo
init_login_db()
init_db()
